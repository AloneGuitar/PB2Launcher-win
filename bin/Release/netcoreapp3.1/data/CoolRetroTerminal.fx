...
#define virtual_resolution float2(800, 600)
...
float4 PS_CoolRetroTerm (float4 pos : SV_Position, float2 uv : TEXCOORD0) : SV_Target
{
	...
	float3 fontColor = origColor;//lerp(saturatedColor,terminalBackColor,0.7 + saturate((contrast *0.3)));
	...
	finalColor = lerp(fontColor.rgb,txt_color,fontColor.rgb);
