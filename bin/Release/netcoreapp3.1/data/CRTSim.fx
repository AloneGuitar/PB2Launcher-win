...
#define RcpScrWidth float2(1.0 / (float)BUFFER_WIDTH, 0.0)
#define RcpScrHeight float2(0.0, 1.0 / (float)BUFFER_HEIGHT)
...
float4 PS_CRTSim_Composite(float4 vpos : SV_Position, float2 uv : TEXCOORD0) : SV_Target
{
	...
	for (int i = 0; i < 3; ++i)
	{
		float2 StepSize = (float2(1.0/BUFFER_WIDTH,0) * (float(i + 2)));
	...
	if (BUFFER_WIDTH == 800)
	{
	if (BUFFER_HEIGHT == 600)
	{
	xyi = 0.0;
	}
	}
	
	Cur_Local = saturate(Cur_Local + (offset * xyi * lerp(float4(1,1,1,1), NTSCArtifact, Tuning_Artifacts)));
